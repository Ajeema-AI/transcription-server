...
class Crawler:
    ...
    async def screenshot(self, project_name):
        ...
        page_metadata = await self.page.evaluate(
        ...
        new_state["browser_session"]["url"] = page_metadata['url']
    ...
    async def go_to_page(self, url):
        try:
            await self.page.goto(url=url if "://" in url else "http://" + url, timeout=30000)
        except TimeoutError:
    ...
    def scroll(self, direction):
        if direction == "up":
            self.page.evaluate(
                "(document.scrollingElement || document.body).scrollTop = (document.scrollingElement || "
        ...
        def crawl(self):
            ...
            def add_to_hash_tree(hash_tree, tag, node_id, node_name, parent_id):
                ...
                value = (True, node_id) if node_name == tag else (True, anchor_id) if is_parent_desc_anchor else (
                    False, None)
            ...
            for index, node_name_index in enumerate(node_names):
                ...
                for tag in ancestor_exceptions:
                    is_ancestor_of_tag, tag_id = add_to_hash_tree(ancestor_exceptions[tag]["ancestry"], tag, index,
                                                                  node_name, node_parent)
